...
if __name__ == "__main__":
    engine = BacktestEngine(symbol="BTCUSDT", capital_per_order=100, leverage=2)    
    df = engine.fetch_historical_data(symbol="BTCUSDT", interval="15m", days=7)
    fvg = smc.fvg(df)
    fvg_signals = fvg["FVG"]
    
    cnt = 12
    for idx in tqdm(range(1, len(df))):
        ts = utils.timestamp_to_utc(df.iloc[idx].name, 8)
        price = df.iloc[idx-1]["close"]
        
        if fvg_signals[idx-1] == 1 and engine.position == 0:
            engine.open_long(ts, price)
            cnt = 12
            
        elif fvg_signals[idx-1] == -1 and engine.position == 0:
            engine.open_short(ts, price)
            cnt = 12
        ...
        if engine.position != 0:
            ...
            if cnt == 0:
                if engine.position > 0:
                    engine.close_long(ts, price)
                else:
                    engine.close_short(ts, price)
